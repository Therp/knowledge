...
class IrAttachment(Model):
        ...
        ir_cron.write(
            ...
            [
                ir_model_data.get_object_reference(
                    cr, uid, module_name, "cron_move_attachments"
                )[1]
            ],
            {
                "nextcall": next_night.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                "doall": True,
                "interval_type": "days",
                "interval_number": 1,
            },
    ...
    def _attachments_to_filesystem_cron(self, cr, uid, context=None, limit=10000):
        ...
        for counter, attachment_id in enumerate(attachment_ids, start=1):
            attachment_data = ir_attachment.read(
                cr, uid, [attachment_id], ["datas", "res_model"], context=context
            )[0]
